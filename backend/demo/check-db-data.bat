...
REM 检查数据库连接
echo [信息] 检查数据库连接...
mysql -u root -p123456 -e "SELECT VERSION();" >nul 2>&1
if errorlevel 1 (
    echo [错误] 无法连接到MySQL，请检查服务是否运行
    pause
    exit /b 1
) else (
    echo [成功] 数据库连接正常
...
REM 检查数据库是否存在
echo [信息] 检查数据库 ai_job_recommender 是否存在...
mysql -u root -p123456 -e "SHOW DATABASES LIKE 'ai_job_recommender';" | findstr "ai_job_recommender" >nul 2>&1
if errorlevel 1 (
    echo [错误] 数据库 ai_job_recommender 不存在
    echo [信息] 正在创建数据库...
    mysql -u root -p123456 -e "CREATE DATABASE IF NOT EXISTS ai_job_recommender CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
    echo [成功] 数据库创建完成
) else (
    echo [成功] 数据库 ai_job_recommender 已存在
)
...
REM 统计数据
echo [信息] 数据统计...
mysql -u root -p123456 -e "USE ai_job_recommender; SELECT COUNT(*) AS '总记录数' FROM jobs;"
